(() => {
  ...
  const els={
    matchTitle:document.getElementById("match-title"),matchMeta:document.getElementById("match-meta"),statusPill:document.getElementById("status-pill"),message:document.getElementById("message"),pin:document.getElementById("admin-pin"),phaseOverview:document.getElementById("phase-overview"),previous:document.getElementById("previous-phase"),next:document.getElementById("next-phase"),closeVoting:document.getElementById("close-voting"),resetMatch:document.getElementById("reset-match"),phaseNote:document.getElementById("phase-note"),phaseStep:document.getElementById("phase-step"),awardEmoji:document.getElementById("award-emoji"),awardTitle:document.getElementById("award-title"),awardHelp:document.getElementById("award-help"),stats:document.getElementById("live-stats"),form:document.getElementById("admin-vote-form"),options:document.getElementById("award-options"),voteButton:document.getElementById("vote-button")
  };
  ...
  function votedKey(matchId,phase,epoch=1){return `athena_h2_admin_v3_voted_${matchId}_epoch_${epoch}_${phase}`;}
  function hasVoted(matchId,phase,epoch=1){return localStorage.getItem(votedKey(matchId,phase,epoch))==="1";}
  function markVoted(matchId,phase,epoch=1){localStorage.setItem(votedKey(matchId,phase,epoch),"1");}
  ...
  function updateVoteButton(){const d=state.data;if(!d?.match)return els.voteButton.disabled=true;els.voteButton.disabled=state.posting||d.match.status!=="open"||d.ready===false||!selectedPlayer()||hasVoted(d.match.matchId,d.phase,Number(d.voteEpoch||1));}
  ...
  function render(data){
    ...
    renderPhases(data);renderOptions(data);renderStats(data);els.form.classList.toggle("hidden",displayPhase===7);els.previous.disabled=state.busy||displayPhase<=1;els.next.disabled=state.busy||displayPhase>=7;els.closeVoting.disabled=state.busy||displayPhase!==6;els.resetMatch.disabled=state.busy;els.closeVoting.classList.toggle("ready",displayPhase===6);const nextSpan=els.next.querySelector("span:first-child");if(nextSpan)nextSpan.textContent=displayPhase===6?"Naar uitreiking":"Volgende fase";updateVoteButton();
  }
  ...
  async function resetMatch(){
    const d=state.data,pin=els.pin.value.trim();if(!d?.match)return;if(!pin)return showMessage("Vul eerst je admin-pincode in.");
    if(!window.confirm("Weet je zeker dat je alle stemmen en resultaten van deze wedstrijd wilt wissen en terug wilt naar fase 1?"))return;
    state.busy=true;clearMessage();
    try{const result=await apiPost({action:"adminResetMatch",matchId:d.match.matchId,pin});showMessage(result.message||"Wedstrijd gereset.",true);await refresh();}catch(err){showMessage(err.message||"Wedstrijd kon niet worden gereset.");}finally{state.busy=false;if(state.data)render(state.data);}
  }

  async function submitVote(event){
    event.preventDefault();const d=state.data,player=selectedPlayer(),epoch=Number(d?.voteEpoch||1);if(!d?.match||!player||hasVoted(d.match.matchId,d.phase,epoch))return;state.posting=true;updateVoteButton();clearMessage();const submissionId=randomId(`admin_phase${d.phase}`);
    try{const result=await apiPost({action:"vote",submissionId,matchId:d.match.matchId,phase:d.phase,browserId:browserId(),player,clientTimestamp:new Date().toISOString()});markVoted(d.match.matchId,d.phase,epoch);showMessage(result.message||"Adminstem opgeslagen.",true);await refresh();}catch(err){showMessage(err.message||"Stem kon niet worden opgeslagen.");}finally{state.posting=false;updateVoteButton();}
  }
  ...
  els.closeVoting.addEventListener("click",closeVoting);els.resetMatch.addEventListener("click",resetMatch);els.form.addEventListener("change",updateVoteButton);els.form.addEventListener("submit",submitVote);
