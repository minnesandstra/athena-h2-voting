(() => {
  ...
  const els = {
    ...
    closeVoting: document.getElementById("close-voting"),
  ...
  function showMessage(text, good = false) {
    ...
    els.message.classList.toggle("good-message", good);
  }

  function clearMessage() {
    els.message.classList.add("hidden");
    els.message.classList.remove("good-message");
  }
  ...
  function currentDisplayPhase(data) {
    return Number(data?.displayPhase || (data?.match?.status === "closed" ? 7 : data?.phase || 1));
  }

  function renderPhases(data) {
    const current = currentDisplayPhase(data);
    els.phaseOverview.innerHTML = "";
    Object.entries(PHASES).forEach(([num, meta]) => {
      const n = Number(num);
      const button = document.createElement("button");
      button.type = "button";
      button.className = `phase-button${n === current ? " active" : ""}${n === 7 ? " ceremony-phase" : ""}`;
  ...
  function renderOptions(data) {
    ...
    if (data.match?.status === "closed") return;
  ...
  function renderStats(data) {
    ...
    if (data.match?.status === "closed") {
      ...
      [
        ["💩 Dick of the Day", awards.dotd],
        ["🔥 Sexy Moment", awards.sexy],
        ["🏆 Man of the Match", awards.motm]
      ].forEach(([label, award]) => {
        const row = document.createElement("div");
        row.className = "stat-row";
        row.innerHTML = `<span>${label}<br><strong>${escapeHtml(award?.winner || "-")}</strong></span><strong>${Number(award?.votes || 0)} stem${Number(award?.votes || 0) === 1 ? "" : "men"}</strong>`;
    ...
    if (!stats.length) els.stats.innerHTML = '<p class="muted">Nog geen live stand beschikbaar.</p>';
  }
  ...
  function render(data) {
    ...
    if (!match) return;
    const displayPhase = currentDisplayPhase(data);
    ...
    els.statusPill.textContent = match.status === "closed" ? "Uitreiking" : (match.status === "open" ? `Fase ${data.phase} open` : (match.statusLabel || match.status));
    els.phaseStep.textContent = displayPhase === 7 ? "FASE 7 · UITREIKING" : `FASE ${data.phase} VAN 6`;
    els.awardEmoji.textContent = PHASES[displayPhase]?.emoji || "🏑";
    els.awardTitle.textContent = PHASES[displayPhase]?.title || data.phaseTitle || "Fase";
    els.awardHelp.textContent = displayPhase === 7 ? "De stemming is gesloten en de definitieve winnaars staan live." : `Live: ${Number(data.voteCount || 0)} stemmen in deze fase.`;
    els.phaseNote.textContent = displayPhase === 7
      ? "Uitreiking actief. Klik op fase 6 als je de stemming opnieuw wilt openen."
      : data.ready === false ? data.readyMessage : `Fase ${data.phase} is klaar om te stemmen.`;

    renderPhases(data);
    renderOptions(data);
    renderStats(data);

    els.form.classList.toggle("hidden", displayPhase === 7);
    els.previous.disabled = state.busy || displayPhase <= 1;
    els.next.disabled = state.busy || displayPhase >= 7;
    els.closeVoting.disabled = state.busy || displayPhase !== 6;
    els.closeVoting.classList.toggle("ready", displayPhase === 6);

    const nextSpan = els.next.querySelector("span:first-child");
    if (nextSpan) nextSpan.textContent = displayPhase === 6 ? "Naar uitreiking" : "Volgende fase";
    updateVoteButton();
  }
  ...
  async function setPhase(targetPhase) {
    ...
    clearMessage();
    try {
      const result = await jsonp({ action: "adminSetPhase", matchId: d.match.matchId, phase: targetPhase, pin });
      if (!result?.ok) throw new Error(result?.message || "Fase kon niet worden gewijzigd.");
      showMessage(result.message || `Fase ${targetPhase} is geopend.`, true);
      await refresh();
    } catch (err) {
  ...
  async function closeVoting() {
    ...
    if (!d?.match) return;
    if (!pin) return showMessage("Vul eerst je admin-pincode in.");
    if (Number(d.phase) !== 6 || d.match.status === "closed") return;
    state.busy = true;
    clearMessage();
    try {
      const result = await jsonp({ action: "adminSetPhase", matchId: d.match.matchId, phase: 7, pin });
      if (!result?.ok) throw new Error(result?.message || "Stemming kon niet worden afgesloten.");
      showMessage("Stemming gesloten. De uitreiking staat nu live op de gewone stemsite.", true);
      await refresh();
    } catch (err) {
      showMessage(err.message || "Stemming kon niet worden afgesloten.");
    } finally {
      state.busy = false;
      if (state.data) render(state.data);
    }
  ...
  els.previous.addEventListener("click", () => {
    ...
    const current = currentDisplayPhase(state.data);
    if (current === 7) setPhase(6);
    else setPhase(Math.max(1, current - 1));
  });
  els.next.addEventListener("click", () => {
    if (!state.data) return;
    const current = currentDisplayPhase(state.data);
    if (current === 6) closeVoting();
    else if (current < 6) setPhase(current + 1);
  });
  els.closeVoting.addEventListener("click", closeVoting);
